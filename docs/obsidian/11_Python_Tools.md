...
Внутри серии `runTests` один раз читает каждый неизменный датасет и передаёт
C++ временный JSON только с выбранным графом. При смене графа используются
записи того же кеша, при изменении файла датасет перечитывается. Для одинаковых
имён выбирается первая запись, как в C++ reader. Ошибочные входы передаются
исходному reader, чтобы сохранить `returnCode` и `stderr`.

Временные файлы удаляются после серии, включая завершение по исключению.
Колонка `flags` сохраняет исходный путь датасета, поэтому повторный запуск
требует доступности исходного файла; временный путь в результаты не записывается.
Одиночный `runTest` по-прежнему получает переданные ему флаги без такой подготовки.
